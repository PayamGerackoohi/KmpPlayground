...
private const val TAG = "Animate Color as State"

class ColorRobotImpl(private val rule: ComposeContentTestRule) : ColorRobot,
    SampleRobot by SampleRobotImpl(rule, TAG) {
    override fun PreviewScope.colorSample() = title {
        content(true) {
            state("State 1")
            performClick()
            state("State 2")
    ...
    override fun CodeScope.colorSample() {
        title(TAG) { performClick() }
        snippet("fun ColorSample() {")
    }
}
