...
private const val TAG = "Animate Float as State"

class FloatRobotImpl(private val rule: ComposeContentTestRule) : FloatRobot,
    SampleRobot by SampleRobotImpl(rule, TAG) {
    override fun PreviewScope.floatSample() = title {
        content(true) {
            state("Opaque")
            performClick()
            state("Transparent")
    ...
    override fun CodeScope.floatSample() {
        title(TAG) { performClick() }
        snippet("fun FloatSample() {")
    }
}
