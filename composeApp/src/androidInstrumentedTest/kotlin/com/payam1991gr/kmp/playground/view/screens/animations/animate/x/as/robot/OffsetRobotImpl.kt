...
private const val TAG = "Animate Offset as State"

class OffsetRobotImpl(private val rule: ComposeContentTestRule) : OffsetRobot,
    SampleRobot by SampleRobotImpl(rule, TAG) {
    override fun PreviewScope.offsetSample() = title {
        content(true) {
            state("State 1")
            performClick()
            state("State 2")
    ...
    override fun CodeScope.offsetSample() {
        title(TAG) { performClick() }
        snippet("fun OffsetSample() {")
    }
}
